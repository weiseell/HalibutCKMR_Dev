...
library(tidyverse) # moved first, just in case one of the 100000000 packages here is trying to take over...
...
print( get( '[<-')) # debug
  ...
  for (y in 2:length(POPY)) {
    ...
    #add the plus group
    N[,y,max(A)] <- N[,SLICE=y,SLICE=max(A)] + 
        N[,SLICE=y-1,SLICE=max(A)] * exp(-Z[,SLICE=y-1,SLICE=max(A)])
...
# Temporary problem with sumover() in RTMB. Until I fix it, we'll do the sum "manually" instead
#  recip_denom_SYL <- 1/sumover(num_Pr_A_SYL,'a')
  ...
  lglk <- sum(dpois(c(n_POP_SYLSYL),c(E_POP_SYLSYL),log = T),na.rm = T)
  ...
  ## Gotta return NEGATIVE lglk :/
return( -lglk)
})


##FOR NOW DISABLE recruitment parameters
system.time( new_f(parm))
